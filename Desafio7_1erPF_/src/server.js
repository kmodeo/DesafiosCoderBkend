...
// POST api/productos con autorizacion authMiddleware para acceso solo de administradores
    ...
    newProductId
        ? res.status(200).json({"success" : "Producto adicionado con el ID: "+newProductId})
        : res.status(400).json({"error": "invalid key. Por favor verifique el contenido del body"})
})
...
routerProducts.delete('/:id', authMiddleware, async (req, res, next) => {
    ...
    wasDeleted 
        ? res.status(200).json({"success": "Producto removido correctamente"})
        : res.status(404).json({"error": "Producto no encontrado"})
})
...
routerCart.post('/', async(req, res) => {
    ...
    newCartId
        ? res.status(200).json({"success" : "Carrito adicionado con el ID: "+newCartId})
        : res.status(400).json({"error": "invalid key. Por favor verifique el contenido del body"})
...
routerCart.post('/:id/productos', async(req,res) => {
    ...
    if (product) {
        ...
        cartExist
            ? res.status(200).json({"success" : "Producto adicionado"})
            : res.status(404).json({"error": "Carrito no encontrado"})
    } else {
        res.status(404).json({"error": "Producto no encontrado, verifique el ID en el body para verificar si es correcto."})
    }
})
...
const server = app.listen(PORT, ()=> console.log(`Server iniciado port ${PORT}`));
